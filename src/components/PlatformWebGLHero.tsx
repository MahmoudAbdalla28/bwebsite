...
export default function PlatformWebGLHero() {
  ...
  useEffect(() => {
    ...
    const screenMat = new THREE.MeshBasicMaterial({
      map: screenGradientSource.texture,
      toneMapped: false,
      side: THREE.DoubleSide,
    });
    const screen = new THREE.Mesh(screenGeo, screenMat);
    screen.position.set(0, 1.0, 0.5);
    ...
    const floorMesh = new THREE.Mesh(floorGeo, floorKeyboardMat);
    ...
    const bloomPass = new UnrealBloomPass(new THREE.Vector2(initialW, initialH), 0.22, 0.42, 0.72);
